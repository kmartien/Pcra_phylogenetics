...
#SBATCH -t 36:00:00   ## walltime in mins, mins:secs, hrs:mins:secs, days-hours 
...
Range_lower=10000				# divergence time range lower limit; default=0
Range_upper=300000				# divergence time range upper limit; default=10000000
Sim_num=30						# number of simulations to run 
...
python ${script} -H ${scriptdir} -P ${PSMC}/psmc -N ${Ne} -l ${Range_lower} -u ${Range_upper} -s ${Sim_num} -p ${THREADS} -o ${OUTDIR}/hPSMC_sim_ ${ProjDir}/${file} > ${OUTDIR}/${species1}_${species2}_simulations.sh
