...
jpeg(paste0("results-raw/map_clades-node", focal.node, ".jpg"), height = 1500, width = 1500)
...
#  geom_point(data = samp.dat, aes(x=Longitude, y = Latitude, colour = clade, size = 3))
  geom_point(data = filter(samp.dat, clade == "other"), aes(x=Longitude, y = Latitude, color = clade)) +
