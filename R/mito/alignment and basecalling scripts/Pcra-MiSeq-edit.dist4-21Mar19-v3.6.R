...
params <- list(
  ...
  run.name = "Pcra-mtdna-MiSeq-R2-edit.dist4",
  folder.name = "~/FASTQ_by_species/Pcra/Pcra_mito/MS2_mitogenomes",
  ref.fname = "ReferenceAssemblyConsensus_40bp_padded_ends.fasta",
  pipeline = "mtdna",
  aligner = "bwa",
  num.threads = 1
)
...
align.params <- list(bwa.max.edit.dist = 4, bwa.max.gap.opens = 2, 
  bwa.max.gap.extensions = -1, bwa.deletion.disallow = 1, bwa.indel.disallow = 0, 
  bwa.seed.length = 35, bwa.max.seed.edit.distance = 2, bwa.num.threads = 1, #params$num.threads, 
...
fasta.params <- list(fasta.min.cov = 5, fasta.min.freq = 0.85, fasta.min.freq.cov = 10)
