...
run.label <- "All_extended_Pcra_refs_min50bp_downsampled.final.recode"
vcf <- read.vcfR(paste0("vcf/",run.label, ".vcf"), convertNA = T)
...
# identify loci with three genotypes (both homozygotes and the heterozygote)
loci.to.keep <- tgt |> 
  group_by(locus) |> 
  distinct(gt_GT_alleles) |> 
  summarise(num.genotypes = n()) |> 
  filter(num.genotypes > 1)
